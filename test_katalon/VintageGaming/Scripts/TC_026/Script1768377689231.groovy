...
TestObject forgotBtn = new TestObject('btn_ForgotPassword')
forgotBtn.addProperty('id', ConditionType.EQUALS, 'forgotPass')
WebUI.click(forgotBtn)

// --- Override prompt to automatically enter registered email ---
WebUI.executeJavaScript("window.prompt = () => 'jorge.arce@example.com';", null)

// --- Click Forgot Password button again to trigger prompt ---
WebUI.click(forgotBtn)

// --- Wait for confirmation alert ---
if (WebUI.waitForAlert(5, FailureHandling.OPTIONAL)) {
    String alertText = WebUI.getAlertText()
    println("Forgot Password confirmation: " + alertText)
    WebUI.acceptAlert()
} else {
    assert false : "Forgot Password confirmation alert did not appear"
}
